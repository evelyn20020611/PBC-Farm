...
class Farm(tk.Frame):  # try
    ...
    def open_store(self): # 點了種子商店按鈕後的function
        ...
        # grid 上去
        #sticky = tk.N+tk.S
        self.button_seed_package_pepper.grid(row = 5, column = 1, columnspan = 2, rowspan = 2, sticky = tk.NE + tk.SW)
        self.button_seed_package_eggplant.grid(row = 5, column = 3, columnspan = 2, rowspan = 2, sticky = tk.NE + tk.SW)
        self.button_seed_package_coriander.grid(row = 5, column = 5, columnspan = 2, rowspan = 2, sticky = tk.NE + tk.SW)
        lab1 = tk.Label(r1, text='青椒種子', height=1, width=15, bg='white', fg='black')
        lab1.grid(row=7, column=1, columnspan = 2, rowspan = 2, sticky = tk.NE + tk.SW)
